  ...
  async ({ task, taskService }) => {
    ...
    if (toBeStatus.type !== 'string' || !ALLOWED_STATUSES.includes(toBeStatus.value))
      taskService.handleBpmnError(
        task,
        'INVALID_TYPE',
        `Status has to be of type string and and of ${ALLOWED_STATUSES.map((s) => `"${s}"`).join(
          ', '
        )}.`
      )
...
client.subscribe(
  ...
  async ({ task, taskService }) => {
    ...
    const prospectName = customerId === undefined ? task.variables.get('ProspectName') : undefined
    const prospectSurname =
      customerId === undefined ? task.variables.get('ProspectSurname') : undefined
    const prospectAddress =
      customerId === undefined ? task.variables.get('ProspectAddress') : undefined
    const prospectEmail = customerId === undefined ? task.variables.get('ProspectEmail') : undefined
